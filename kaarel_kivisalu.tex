...
\begin{equation}
    ...
    \label{ootev}
\end{equation}
kus jälg (ingl \textit{trace}) $\tr(\op{A}) \equiv \sum_n \braket{n|\op{A}|n}$\footnote{\url{http://en.citizendium.org/wiki/Trace_\%28mathematics\%29} lõputu dimensioon converges} \parencite{viide}. %$n\times n$ ruutmaatriksist on defineeritud kui $\tr(A) \equiv \sum_{i=1}^n a_{ii}$, kus $a_{ii}$ on maariksi $A$ $i$-nda rea ja $i$-nda veeru element.
...
Siin osas vaadeltakse harmoonilist ostsillaatorit, millele on lisatud kuuphäiritus, gravitatsiooniväljas. Selleks, et leida häirituse mõju süsteemile lahendatakse kõigepealt omaväärtusprobleem harmoonilise ostsillatori jaoks gravitatsiooniväljas. Sellele vastav hamiltoonian on
\begin{equation}
    \op[0]{H}=\frac{\op{p}^2}{2m}+\frac{m\omega^2 \op{x}^2}{2} + mg\op{x}=\hbar \omega (A A^\dagger + 1/2)-k,
\end{equation}
kus $k=\frac{mg^2}{2\omega^2}$ ja
\begin{align}
    \begin{split}
...
\begin{equation}
    \mathcal{H} = \frac{\op[0]{H}}{\hbar \omega}=(A^\dagger A + 1/2)-\frac{k}{\hbar \omega}.
    \label{mathcalH}
\end{equation}
...
\begin{equation}
    \left(\mathcal{H}-1/2 + \frac{k}{\hbar \omega} \right)\ket{\varepsilon_0}=0
\end{equation}
või
\begin{equation}
    \mathcal{H}\ket{\varepsilon_0}=\left(\frac{1}{2} - \frac{k}{\hbar \omega}\right) \ket{\varepsilon_0}
\end{equation}
või
\begin{equation}
    \varepsilon_0=\frac{1}{2} - \frac{k}{\hbar \omega}.
\end{equation}
...
\begin{equation}
    \varepsilon_n=\left(n+\frac{1}{2}\right)-\frac{k}{\hbar \omega}, \qquad n=0, 1, 2,\dots
\end{equation}
või
\begin{equation}
    E_n=\hbar \omega \left(n+\frac{1}{2}\right)-k, \qquad n=0, 1, 2,\dots
\end{equation}
Nüüd tahetakse leida võrranditest \eqref{cepsilon} ja \eqref{cepsilon1} konstandid $C_\varepsilon$ ja $C_{\varepsilon+1}$. Kuna $\varepsilon=n+1/2 - k/\hbar \omega$, tähistame kette täisarvuga $n$. Tahetakse leida konstant $C_n$ järgmisest võrrandist:
\begin{equation}
    A\ket{n}=C_n \ket{n -1}.
    \label{konstket}
\end{equation}
...
\begin{equation}
    ...
    \label{konstbra}
\end{equation}
Kombineerides võrrandid \eqref{konstket} ja \eqref{konstbra} saadakse, te
\begin{align}
    \begin{split}
        \braket{n|A^\dagger A|n}&=\braket{n-1|C_n^* C_n| n-1} \\
        \braket{n|\mathcal{H}-\tfrac{1}{2}+\tfrac{k}{\hbar \omega}|n}&=C_n^* C_n \\
        \braket{n|n|n}&=|C_n|^2 \\
        |C_n|^2&=n \\
        C_n&=\sqrt{n} e^{i\phi}.
    \end{split}
\end{align}
Kuna $\psi$ väärtus on vabalt valitav, siis on mugav võtta selle väärtus nulliks. Siis saadakse, et
\begin{equation}
...
%    \op[1]{H}=mg\op{x}
%\end{equation}
%Lahendid saab leida häirituse meetodiga. On teada potentsiaali
%\begin{equation}
